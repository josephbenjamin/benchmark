...
df_xlsx = pd.read_excel('data/20250130 - Gilts in Issue.xlsx', sheet_name="Sheet1", header=None)
...
df_conventional.insert(0, "Maturity Bucket", None)
...
for label in maturity_labels:
    if label in maturity_rows:
        ...
        if previous_label is not None:
            df_conventional.loc[previous_label:row_idx-1, "Maturity Bucket"] = previous_label_label
        previous_label = row_idx
        previous_label_label = label

# Assign the last maturity bucket
if previous_label is not None:
    df_conventional.loc[previous_label:, "Maturity Bucket"] = previous_label_label
...
print(data)
print(data['Total Amount in Issue \n(£ million nominal)'].sum())
